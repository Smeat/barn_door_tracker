...
void dht_update(dht22_t* sensor, uint32_t time) {
	switch(sensor->state){
		case DHT_IDLE:
			// only update every 2 seconds
			if(time - sensor->last_update > 2000000) {
				sensor->write_pin(1);
				sensor->state = DHT_REQUESTING;
				sensor->last_update = time;
			}
			break;
		case DHT_REQUESTING:
			if(time - sensor->last_update > DHT_MIN_HIGH_US) {
				// just to set to input mode and enable pullup
				sensor->read_pin();
